...
1. **Tiered keyword search** extracts keywords from the task description using multi-channel decomposition: raw keywords, camelCase/snake_case splitting, and equivalence class expansion via a concept thesaurus (e.g., "auth" expands to include "authentication", "authorize"). These keywords locate seed nodes for the graph walk.
2. **BM25 over FTS5** scores candidate nodes by keyword relevance. BM25 (Best Match 25) is a term-frequency scoring algorithm; FTS5 is SQLite's full-text search engine, used here to index node names and docstrings for fast keyword lookup.
3. **Density-adaptive RWR walk** propagates relevance from those seed nodes across the graph. Random Walk with Restart (RWR) follows edges with probability (1 - alpha) and restarts at seed nodes with probability alpha. "Density-adaptive" means the system auto-adjusts seed count and seed selection strategy based on graph size: above 40K nodes it prefers type seeds (interfaces, structs) as entry points and increases the seed count, preventing keyword flooding in dense graphs.
