...
When a phrase matches with high confidence (weight >= 0.9, source "framework"), the matched symbols bypass RWR scoring and inject directly at the top of the ranked list (forced injection). Learned vocabulary associations (source "learned") compete through RRF instead (soft injection), preventing displacement of correct results on well-covered tasks. This solves the vocabulary gap without broadening BM25 (which floods results with noise on large repos). Language scoping ensures framework classes only fire on repos using that framework. Learned vocab associations accumulate from agent usage: when an agent uses a symbol after a context query, the keyword -> symbol association is recorded (filtered by `isVocabWorthy` to exclude common English words). After 2+ observations, it becomes an automatic equivalence class with confidence-weighted RRF scoring. Cross-task bridging validated: task A's vocab helps task B via shared keywords.
...
- P@10 = 0.321 cold start (291 tasks, 16 repos, 8 languages, 13 self-adapting mechanisms, honest measurement)
- Competitive: 3.69x codegraph, 5.84x GitNexus, 6.17x Gortex, 13.96x Aider, 21.4x grep (cold start)
...
Implicit feedback tracks which symbols were returned but never used by the agent (negative signal) and which the agent subsequently referenced in tool calls (positive signal). Feedback is scoped to keyword clusters (per-cluster scoping prevents cross-task interference). When agents use symbols, keyword -> symbol associations are recorded (noise-filtered); after 2+ observations, these become learned equivalence classes with confidence-weighted RRF injection. Cross-task bridging: vocab learned from one task helps different tasks via shared keywords. Feedback automatically expires when the package's SubgraphRoot changes (code was modified).
...
|---|---|---|
| P@10 (precision at 10) | 0.321 | 16 repos, 291 tasks, 13 self-adapting mechanisms |
