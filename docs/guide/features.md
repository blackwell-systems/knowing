...
Generated: 2026-05-15 (updated: 2026-05-27, features 77-152 added previously; features 153-175 added: embedding gap-fill seeds, knowing enrich embeddings, brute-force vector search from SQLite, parallel benchmark harness, GraphNodeCount per-engine field, nomic-embed-text default model, BENCH_GAP_THRESHOLD, round 2 per-task logging, spark-java fixtures expanded, two-phase gopls warmup, kubernetes/terraform enrichment results, caddy/fastapi/ocelot corpus repos, skip test/generated in edge upgrade, package-sorted edges, RealNodeCount, corpus expansion 13 repos/257 tasks/8 languages, task memory compounding, platform deployment, Makefile targets, similarity OOM fix, adaptive retrieval architecture doc)
