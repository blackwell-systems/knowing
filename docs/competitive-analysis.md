...
knowing is an intelligence versioning system: Git versions files, knowing versions the understanding of code. Every entity (symbols, relationships, files, repos, and graph snapshots) is identified by its SHA-256 hash. Each snapshot captures not just code structure but the system's accumulated intelligence: relationship confidence, provenance tiers, feedback signals, and what changes mean. It indexes code via 25 extractor types covering 12 programming languages + 13 infrastructure/cloud formats, enriches edges via multi-language LSP (gopls, typescript-language-server, pyright, jdtls, rust-analyzer, OmniSharp) and SCIP, ingests runtime traces via OTLP, and exposes the graph through 23 MCP tools + 3 prompts. ~60K LOC Go, single binary, SQLite. Published as v0.2.0 across Homebrew, npm, PyPI, Docker, and MCP Registry. It targets AI coding agents and platform teams that need versioned, provenance-scored code intelligence, not just file contents.
