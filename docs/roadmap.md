...
| 17 | **Embedding gap-fill seeds (BM25 fallback)** | **HIGH PRIORITY.** When BM25 returns < 5 seeds, query sqlite-vec ANN for supplemental embedding-based seeds at lower RRF weight (1.0 vs BM25's 2.0). This is NOT BM25 replacement (which risks regressing kafka 0.337, terraform 0.265, caddy 0.255 where BM25 works). It specifically fills the gap on tasks where BM25 finds nothing (django's 42% zero-rate: "validate the request body" would find `FormValidator.clean()` via semantic similarity even with zero keyword overlap). Session 15 tested embeddings as Channel 3 (RRF fusion, neutral: same symbols as BM25). Session 16 tested BM25 gap injection (too noisy, reverted). The untested combination is embedding-based gap candidates (higher precision than BM25 for vocabulary-gap tasks). **Cannot regress repos where BM25 already works** because embedding seeds only enter when BM25 is weak. **Prerequisites: items #19 (pre-embed all nodes) and #20 (sqlite-vec).** | Experiment, HIGH |
| 19 | **Pre-embed all nodes** | New `knowing enrich embeddings` command that batch-embeds all nodes after indexing. 100K nodes * 32 batch size * ~200ms/batch = ~10 min. Eliminates cold-start penalty (660ms -> 220ms on first query). Enables sqlite-vec ANN search across full graph. Incremental: only embed new/changed nodes. Daemon mode: embed in background after each reindex. Prerequisite for item #17 (vector-based seed selection needs all vectors, not just cached query results). | 10 min one-time cost, instant re-ranking, enables vector seeds | Medium |
| 20 | **sqlite-vec integration** | Add `sqlite-vec` extension for ANN (approximate nearest-neighbor) vector search directly in SQLite. Currently embedding vectors are stored as raw blobs and cosine similarity runs in Go (fine for 50-100 candidates, unusable for 100K+). sqlite-vec enables: (a) embedding-based seed selection in ~1ms across all pre-embedded vectors, (b) gap injection via "find semantically similar symbols not in the RWR set", (c) sub-millisecond vector lookups at scale. Requires item #19 (pre-embed) to have vectors for all nodes. Required infrastructure for item #17 (BM25 replacement). | Infrastructure |
