...
|---|------|-----|--------|-----------------|
| 1 | ~~Task-noise correlation: per-cluster tracking~~ | **SHIPPED session 25.** keyword_cluster column on feedback table, derived from sorted primary keywords. RecordFeedback and FeedbackBoosts both accept cluster scope. Django 5-round compounding: R@10 +5.2% (0.347->0.365), MRR +12.6% (0.310->0.349), round 5 regression eliminated (0.194 vs 0.192 without clusters). Cold start neutral (0.278). Also fixed release.yml (eval timeout blocking GHCR since v0.7.0) and missing short-mode skips in bench tests. | **Shipped** | Learning robustness |
| 2 | ~~Task-noise correlation: weighted negatives~~ | **Tested session 25: 4-mode sweep (none/sqrt/linear/asym) on 5-round Django compounding.** Cluster-only (no weighting) wins: best MRR (0.349), best R@10 (0.365). All weighted modes hurt MRR because dampened negatives slow learning. Cluster scoping alone solved the cross-task interference problem that motivated weighting. Infrastructure kept (`BENCH_FEEDBACK_WEIGHT` env var) but default is `none` (raw scoring). | **Tested** | Not needed |
| 3 | ~~Vocabulary expansion from usage~~ | **SHIPPED session 25.** Write path: `recordImplicitFeedback` records (keyword, symbolName, symbolHash) in `vocab_associations` table when `FlushAll` reports positively attributed symbols. Read path: `ForTask` and `equivSearch` query `LearnedVocabTargets` with `minCount=2`. Unit test proves concept: unfindable symbol becomes findable after 2 associations. Django compounding: R@10 peak 0.376 (round 3), P@10 stable. Migration 021. | **Shipped** | Learned equiv classes |
| 3a | ~~Cross-task vocab validation~~ | **SHIPPED session 26.** `TestCrossTaskVocab` benchmark with per-task attribution. Django +41.4% in isolation, full corpus 0.0% aggregate (safe). 100% of improvements are cross-task. Noise keyword filter (`isVocabWorthy`), soft RRF injection (not forced), confidence weighting (count scales 0.3-0.8). 10-round full corpus compounding: P@10 peak +2.2%, MRR peak +8.1%. `debug-vocab -task` flag for filter preview. Mechanism #13 added. | **Shipped** | Validated |
| 3b | ~~MCP server vocab wiring~~ | **SHIPPED session 25.** `ObserveToolUse` now records vocab associations via `UsedSymbolNames` + `RecordVocabAssociation`. Keywords stored on `Server.lastTaskKeywords` from most recent `context_for_task`. Both engine and MCP paths now record vocab. | **Shipped** | Production completeness |
| 3c | **Vocab association expiration** | Old associations should decay when symbols are renamed or deleted. Tie to Merkle expiration (like feedback does with `neighborhood_root`): associations whose symbol's SubgraphRoot changed are invalidated. | Low | Correctness |
| 3d | ~~Vocab diagnostics~~ | **SHIPPED session 25.** `knowing debug-vocab`, `debug-feedback`, `debug-equiv` CLI commands. | **Shipped** | Observability |
| 4 | ~~Proximity-weighted packing~~ | **SHIPPED session 24.** RWR score as proximity factor in packing: `density * sqrt(rwrScore)`. Zero extra cost. Full corpus neutral (0.277 vs 0.279). Enriched saleor: 0.209 (was 0.182), regression halved from -23% to -11%. BFS approach tested first but too expensive (2x latency, 25% memory), reverted. RWR proxy is the winner. | **Shipped** | Enrichment regression halved |
| 5 | **Incremental RWR (Merkle-cached walks)** | Cache `(seedHashes, relevant SubgraphRoots) -> RWR scores`. On repeat queries or small code changes, most subgraphs are unchanged so the walk is a cache hit. Structurally guaranteed correct (not TTL-based). Turns 2ms query into <0.5ms for repeated work. In `--watch` mode, only changed packages get re-walked. **Architectural moat: competitors need full CAS rewrite to match.** | Medium | Latency, moat |
| 6 | **Delta context packing** | Agent has pack P1 (PackRoot R1). Code changes. New pack P2 shares 90% of symbols. Send Merkle diff instead of full 5000-token pack: "remove X, Y; add Z." Incremental context for iterative work. 80-90% token savings on subsequent queries. PackRoot and Merkle structure identify exactly which symbols changed. **Architectural moat: string-matching systems can't diff results structurally.** | Medium | Token efficiency, moat |
| 7 | ~~Change-aware scoring~~ | **SHIPPED session 25.** `commitRecencyScore` uses Node.LastCommitAt from git blame: +0.05 (day), +0.03 (week), +0.01 (month), 0 (older/absent). Returns 0 when blame data absent (benchmark neutral). Activates in production with blame enrichment. Lighter approach than SubgraphRoot comparison (no tree rebuild needed). | **Shipped** | Precision, recency |
| 8 | **Deploy platform API** | api.blackwell-systems.com. DigitalOcean Droplet, Cloudflare Tunnel. DEPLOY.md + deploy.sh committed. | Low | Live product |
| 9 | ~~Documentation sweep~~ | **SHIPPED session 24.** context-engine.md, retrieval-pipeline.md, introduction.md updated. Task memory -> implicit feedback. Blog already done. | **Shipped** | Accuracy |
...
| 12 | **Zero-task audit cycle** | Session 25 audit: 14/36 Django tasks score 0.00 (39%). Categorized into 4 patterns: (A) empty/degenerate FTS query, 3 tasks; (B) wrong neighborhood seeds, 5 tasks; (C) true vocabulary gap, 4 tasks; (D) disambiguation failure, 2 tasks. Fixes below target patterns A-D. | Medium | +0.01-0.02 P@10 |
| 13 | ~~Re-test enriched saleor with exponent 0.3~~ | **Tested session 24: P@10 = 0.182 (same as no-proximity enriched).** Exponent 0.3 is too gentle for saleor's extreme phantom ratio (3x). 0.5 was better (0.209). Optimal exponent is repo-dependent: 0.3 for normal enriched repos, higher for extreme phantom ratios. Future: adaptive exponent based on phantom-to-real node ratio. Saleor stays unenriched. | **Tested** | Negative |
| 14 | ~~Adaptive proximity exponent~~ | **SHIPPED session 25, tested on enriched saleor: P@10 = 0.182 (same as fixed 0.3).** The adaptive exponent is correct in packing but phantoms get filtered before packing reaches them, so ratio is always ~0 in the ranked candidates. The enrichment regression is caused by phantom dilution in the **RWR walk itself**, not in packing. Fix needs to be upstream: dampen phantom weight in adjacency map or filter phantoms from walk. Adaptive exponent still ships (no regression on corpus P@10=0.280). | **Shipped (packing neutral, walk is the real issue)** | Saleor enrichment still regresses |
| 14a | ~~RWR phantom dampening~~ | **Tested session 25: P@10 = 0.182 (no change).** Filtered edges to/from externals in both BFS and cache paths. No effect because externals were already excluded from BFS frontier expansion and act as dead ends that redistribute to seeds. The regression is NOT from phantom probability sinks. **Root cause identified:** enrichment adds edges that boost non-GT real symbols' centrality (e.g., `order_cancelled`, `ORDER_CANCELLED` webhook/plugin symbols rank above GT `Order.can_cancel`). The fix is in equiv classes or vocab expansion, not in the walk. | **Tested** | Wrong diagnosis |
| 14b | ~~Enrichment centrality rebalancing~~ | **SHIPPED session 25.** LSP edge weight attenuation: `lsp_resolved` provenance edges get 0.3x weight in RWR walk. 4-point sweep on enriched saleor: 0.3=0.218 (best), 0.5=0.182, 0.7=0.200, 1.0=0.182. Enrichment regression halved from -22.9% to -7.6%. Full corpus: 0.283, 0.279 (two runs, neutral/positive vs 0.280 baseline). Default set to 0.3. Override with `BENCH_LSP_EDGE_WEIGHT`. | **Shipped** | Enrichment regression halved |
| 15 | **More framework-using repos** | Saleor proved equiv classes generalize. Add a Flask app, Rails app, Spring Boot service. Each adds 10-20 tasks that score well on existing equiv classes. Improves eval credibility and may lift aggregate by representation. | Medium | Eval credibility, P@10 |
| 16 | **Extract benchmark to standalone repo (CRET)** | Session 26 audit: 18 files copy directly (zero knowing imports), 5 trivial decouples (1 import each), adapters/knowing.go stays in knowing. ~2 hours estimated. Also: context packing benchmark (`bench/context-packing/`) extractable as second standalone benchmark. Proposal updated: `docs/proposals/code-retrieval-eval-toolkit.md`. | Low | Credibility |
| 17 | ~~FTS fallback decomposition~~ | **SHIPPED session 25.** Django P@10: 0.194 -> 0.203 (+4.6%). django-medium-103 cracked 0.00 -> 0.40. Full corpus 0.278 (neutral). | **Shipped** | +4.6% Django P@10 |
