...
| 14 | ~~Adaptive proximity exponent~~ | **SHIPPED session 25.** `adaptiveProximityExponent` computes phantom-to-real ratio from packing candidates: `clamp(0.3 + 0.2 * ratio, 0.3, 0.7)`. Normal repos (ratio ~0): 0.3. Saleor (ratio ~3): 0.7. Zero cost (no DB query, computed from ranked slice). `BENCH_PROXIMITY_EXP` still overrides for sweep testing. | **Shipped** | Fix saleor enrichment |
