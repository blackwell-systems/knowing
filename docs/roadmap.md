...
| 19a | **Parallel benchmark without embedding regression** | `BENCH_PARALLEL=1` loses ~0.022 P@10 from ONNX CPU contention (session 19 measured only -0.001, better than expected). **ONNX contention fixes:** (1) Mutex around ONNX inference: parallel BM25/RWR/packing, serialize re-rank. (2) Pre-compute re-rank scores: embed task descriptions upfront. (3) `OMP_NUM_THREADS=1`: single-threaded inference per task. **Embedding I/O fixes:** (4) Load vectors once at engine init: cache in memory, eliminate per-task SQLite reads. Biggest parallel win. (5) Serialize HNSW index to `.knowing/hnsw.idx`: load O(1) vs rebuild O(n log n). (6) `PRAGMA mmap_size`: memory-map embedding tables, eliminate read syscalls. One line change. | Infrastructure |
