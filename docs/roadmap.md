...
| A5 | **Multi-agent feedback semantics** | Define and document how the learned feedback / noise-demotion layer behaves when 2+ agents or harnesses share one graph: shared feedback (needs merge/coordination) vs per-agent namespaces. | The content-addressed graph is deterministic and safely shared, but the feedback layer is stateful. Concurrent agents writing feedback to one graph is a last-writer hazard (same class as shared-memory concurrency). Needs a defined model before shared use. | Medium | Not started |
