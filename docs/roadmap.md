...
| 7c | **MRR improvement (exact-match anchor)** | Codegraph's MRR is 0.459 vs knowing's 0.411 (Run 23). Root cause hypothesis: RRF fusion dilutes the top position by averaging across channels. If tiered search has an exact name match at rank 1, it should be pinned to position 1 in final output regardless of other channels. Codegraph gets this right because it doesn't fuse: exact match = done. | +0.05 MRR | Investigate |
