...
| 7 | **Task-noise correlation system** | Reframe "task memory" as noise suppression, not recall. The system observes which symbols are consistently returned but never used across similar tasks, and learns to suppress them. Current implicit feedback (session 24) does a crude version: returned but not used = negative. Early results: Django P@10 0.186 -> 0.192 after 2 rounds with implicit feedback wired into the engine. Three components: | Medium | Compounding |
