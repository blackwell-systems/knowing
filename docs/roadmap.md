...
| 7a | **Co-location scoring** | If multiple candidate symbols share a file, boost all of them. Learned from codegraph (Run 23): their co-location heuristic helps when a task maps to multiple symbols in the same module (e.g., "caching middleware" -> `CacheStore` + `Middleware` in `cache.go`). Add as a signal in `RankSymbols`: +0.05 per co-located pair. | +1-3pp P@10 (estimated) | Planned |
| 7b | **Per-file diversity cap** | No more than 3 symbols from the same file in the top-10 results. Prevents one file from dominating when all its symbols score similarly (common in flat graphs). Codegraph caps at 20% per file; we have no cap currently. | Better coverage, +MRR | Planned |
| 7c | **MRR improvement (first-result quality)** | Codegraph's MRR is 0.459 vs knowing's 0.411 (Run 23). Their first result is more often correct. Investigate: is RRF fusion diluting the top position by averaging across channels? Should the tiered channel get a bonus for exact matches at rank 1? | +0.05 MRR | Investigate |
