...
| 3a | **Cross-task vocab validation** | Current compounding test runs the same tasks each round, so vocab is self-reinforcing. Need a test with deliberately overlapping task pairs (task A's associations help task B). Validates the cross-task vocabulary bridging that is the primary value proposition. | Low | Validation |
...
| 3d | **Vocab diagnostics** | `knowing debug-vocab -db <path>` command to inspect learned associations: keyword, symbol, count, last_seen. Useful for understanding what the system has learned and debugging false associations. | Low | Observability |
...
| 14b | **Enrichment centrality rebalancing** | The real saleor enrichment regression: pyright creates edges that boost webhook/event handler symbols above GT implementation symbols. 100% recall at rank 18-38 but P@10=0. Options: (1) saleor-specific equiv classes for common task patterns, (2) downweight LSP-enriched edges in RWR (0.3x vs 1.0x for tree-sitter), (3) rely on vocabulary expansion (#3) to learn the right associations after agent usage. | Medium | Fix saleor enrichment regression |
...
| 16 | **Extract benchmark to standalone repo** | `blackwell-systems/context-retrieval-benchmark` created (session 24). Needs: extract harness, tasks, adapters from knowing. Clean adapter interface as Go module. Enables community contributions and independent reproduction. Do before enterprise pitches or academic submissions. | Medium | Credibility |
| 17 | **FTS fallback decomposition** | Zero-audit Pattern A fix. When `buildFTSQuery` yields 0 BM25 results, decompose compound keywords into OR'd components: `"ModelAdmin.get_inlines"` -> `ModelAdmin OR get_inlines`, `"html.escape"` -> `html OR escape`. Currently the pipeline silently produces empty seeds. ~30 lines in `ForTask`. Fixes swe-002, swe-003, swe-005. | Low | +0.08 Django P@10 (3 tasks) |
