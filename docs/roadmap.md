...
| 14 | **Adaptive proximity exponent** | Exponent 0.3 is optimal for normal enriched repos but too gentle for extreme phantom ratios (saleor 3x). Compute phantom-to-real ratio at query time: `phantomCount / realNodeCount`. If ratio > 2x, use higher exponent (0.5-0.7). O(1) with cached counts. Would make enrichment net-positive on saleor without regressing other repos. | Low | Fix saleor enrichment |
