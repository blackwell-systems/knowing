...
| 7a | ~~**Co-location scoring**~~ | ~~If multiple candidate symbols share a file, boost all of them.~~ **Tested, REJECTED.** Regressed P@10 from 0.256 to 0.212 (-17%) on Flask+Django+Cargo. RWR already captures co-location implicitly through graph connectivity (co-located symbols call each other). Adding an explicit signal double-counts. | Negative | ~~Planned~~ |
| 7b | ~~**Per-file diversity cap**~~ | ~~No more than 3 symbols from the same file in top-10.~~ **Tested, REJECTED.** Regressed P@10 from 0.256 to 0.221 (-14%) combined with 7a. Ground truth often has 4-5 relevant symbols in the same file (Django query.py, Flask scaffold.py). Capping discards correct results. | Negative | ~~Planned~~ |
| 7c | ~~**MRR improvement (exact-match anchor)**~~ | ~~Pin tiered exact match to position 1 post-fusion.~~ **Tested, REJECTED.** MRR improved +0.018 (0.400->0.418) but P@10 regressed -16% (0.256->0.214). The literal name match isn't always the best result; structurally important symbols (high RWR) can be more relevant. Forcing literal matches to #1 displaces better results. The MRR gap vs codegraph may not be fixable without trading precision. | Negative | ~~Investigate~~ |
...
| 10 | **RWR query latency on large graphs** | On k8s (268K edges), RWR takes ~10s. codegraph queries in ~1s (BM25 only, no graph walk). Need to close this gap without losing the precision advantage of graph-based ranking. Four approaches below. | Query latency | P1 |
| 10a | **RWR early termination** | Stop iterating when top-K scores stabilize (delta < epsilon between iterations). On large graphs, convergence often happens by iteration 5-8, not the fixed 20. Quickest win, no architectural change. | -50% latency (est.) | Planned |
| 10b | **BFS depth limit on adjacency map** | Instead of loading ALL reachable edges, only load edges within N hops of seeds. If N=3, the walk covers the structural neighborhood without touching the entire graph. Trades recall for speed. | -70% latency (est.) | Planned |
| 10c | **Pre-computed adjacency cache** | Build the adjacency map once at index time, store in SQLite. RWR loads from cache instead of querying edges one-by-one. Shifts cost from query time to index time. | -80% latency (est.) | Planned |
| 10d | **Community-scoped RWR relaxation** | Currently only activates when ALL seeds cluster in 1 community. Relax: if >60% of seeds are in the same community, constrain the walk to that community's subgraph. Reduces reachable edges on large repos. | -40% latency (est.) | Planned |
