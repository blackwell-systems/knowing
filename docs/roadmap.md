...
| 3b | **MCP server vocab wiring** | MCP server's `DetectUsed` path (server.go:310) records feedback directly on the store, bypassing the engine's `recordImplicitFeedback`. Need to ensure the MCP path also records vocab associations when positive attribution is detected. | Low | Production completeness |
