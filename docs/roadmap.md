...
| 17c | **Embedding re-ranker regression on specific repos** | Confirmed on TWO repos (session 19): **Cargo** 0.268 -> 0.214 (-20% with embeddings), **Homebrew** 0.275 -> 0.200 (-27%). Not limited to unenriched repos (cargo has full rust-analyzer enrichment with 9K+ lsp_resolved edges). The re-ranker helps most repos (+17% aggregate) but actively hurts some. The aggregate P@10 is being held back by these regressions. **Possible fixes:** (1) per-repo adaptive re-ranker: measure re-ranker delta during first few queries, disable if negative. (2) confidence-weighted re-rank: only re-rank candidates above a graph-score threshold. (3) skip re-rank when graph walk already has high-confidence results. **Highest-impact open item.** | Investigation (HIGH) |
