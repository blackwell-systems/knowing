...
| 22a | **Homebrew corpus repo (blocked)** | 278K LOC Ruby, 8,476 nodes, density 15.2. Tree-sitter P@10 = 0.275 (no embeddings). 20 fixtures written. **Blocked on Ruby LSP enrichment:** ruby-lsp requires inclusion in project Gemfile (bundler coupling), solargraph starts but produces zero lsp_resolved edges (can't resolve symbols without `.solargraph.yml` or WorkDir pointing at `Library/Homebrew/`). Embedding re-ranker regresses this repo (0.275 -> 0.200), first observed case. **Unblock path:** solargraph with WorkDir config, or `BUNDLE_GEMFILE` env var for ruby-lsp. Do not add to corpus until enrichment works and score exceeds corpus average. | Blocked |
