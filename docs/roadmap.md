...
| 7 | **Change-aware scoring** | Symbols in packages where SubgraphRoot differs from previous snapshot get a scoring boost. "Developer is working in code that recently changed." O(1) per package (compare two hashes) vs O(edges) for traditional diff. Different from git-blame recency: tracks structural relationship changes, not authorship. Quick win with existing infrastructure. | Low | Precision, recency |
