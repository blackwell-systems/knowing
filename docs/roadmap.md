...
| 18 | **Action-verb symbol extraction** | Zero-audit Pattern A fix. Words after action verbs with parens/dots are symbol references, not English verbs: `.delete()` -> extract `delete` as symbol keyword. Currently `delete` is filtered by `actionVerbs` map. ~10 lines in `extractKeywordSet`. Fixes swe-002 and similar tasks. | Low | +0.03 Django P@10 (1-2 tasks) |
| 19 | **Test symbol seed deprioritization** | Zero-audit Pattern B fix. 21% of returned results are test symbols (kind=test or QN contains `/tests/`). Test symbols make poor seeds because they test behavior but don't implement it. Deprioritize in BM25 seed ranking by checking QN path. ~15 lines. Fixes medium-101, edge-003, and similar. | Low | +0.05 Django P@10 (2-3 tasks) |
| 20 | **Package-path constrained seed selection** | Zero-audit Pattern B/D fix. Task descriptions contain path hints: "admin changelist" -> `django/contrib/admin/`, "template rendering" -> `django/template/`, "SQL compiler" -> `django/db/models/sql/`. Map domain terms to package paths and boost seeds from matching packages. ~50 lines. Fixes easy-200, medium-102, swe-004. | Medium | +0.05 Django P@10 (2-3 tasks) |
