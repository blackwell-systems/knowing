...
knowing already works well as a Claude Code MCP server (28 tools, GCF output, `--watch`). These items make it a first-class citizen in *any* agentic harness (Hermes, OpenCode, Codex, custom loops), where the goal is ambient context injection (hooks push context in, the agent never has to pull) and shared use across multiple harnesses over one graph. A1 and A2 (the two highest-leverage items) are **shipped**; the rest is polish or can be worked around harness-side.

| # | Item | What it does | Why | Effort | Status |
|---|------|-------------|-----|--------|--------|
| A1 | **Harness-agnostic hook contract** | A generic `knowing hook <event>` command with a documented, harness-neutral stdin/stdout contract (event + payload in, context out), plus a built-in `--emit claude-code` adapter and per-harness examples. Decouples the benchmarked hook logic (edit-symbol extraction, filename fallback) from Claude Code's `SessionStart`/`PreToolUse` + envelope format. | Today every non-Claude-Code harness needs a bespoke adapter to get auto-injected context. A neutral contract makes wiring knowing into Hermes/OpenCode/Codex a config task, not a code task. The single biggest "easy in any harness" win. | Medium | ✅ Done (`cmd/knowing/hook.go`, `docs/architecture/hooks-integration.md`, `hooks/adapters/`) |
| A2 | **HTTP / streamable-http MCP transport** | `knowing mcp --http [--port <p> | --addr <a>]` alongside the current stdio transport. | stdio-only means one client per process and no remote/shared instance. HTTP lets a box-hosted agent, spawned subagents, or multiple harnesses hit one warm instance. Content-addressing already lets each agent run local; HTTP covers the remote/shared case. | Medium | ✅ Done (`cmd/knowing/mcp.go`, wires existing `Server.ServeHTTP`) |
| A3 | **Git-native auto-sync hook** | `knowing sync` reindexes a tracked repo (scoping LSP enrichment to files changed since the last indexed commit); `knowing sync --roster` does every tracked repo; `knowing sync install` writes non-destructive git post-merge + post-checkout hooks (branch-switch only) that call it. | The `--watch` fs-event path assumes a long-lived process on the working tree. An agent that pulls repos needs the graph to refresh on pull with zero agent action, so "stay in sync" is turnkey instead of a per-harness cron recipe. | Low | ✅ Done (`cmd/knowing/sync.go`) |
| A4 | **Core agent toolset preset** | A small high-signal default profile (`context_for_task`, `blast_radius`, `test_scope`, `explain_symbol`) with the full 28 tools opt-in. | 28 tools is a large surface for an agent to reason over. A curated default (plus hooks, so the agent rarely picks a tool at all) reduces tool-selection overhead and mis-selection. | Low | Not started |
| A5 | **Multi-agent feedback semantics** | Ephemeral attribution state (keywords, returned packs, session boosts, implicit-feedback tracker, GCF dedup) is now per-connection (keyed by MCP session id, evicted on disconnect), fixing a concurrent-map panic and cross-agent attribution corruption. Durable feedback stays pooled (collective learning) but carries client/harness provenance, so per-agent read policies need no migration. Threat model (confidence weighting + Merkle expiration) documented. | The content-addressed graph is deterministic and safely shared, but the feedback layer is stateful. Concurrent agents writing feedback to one graph is a last-writer hazard. | Medium | ✅ Done (`internal/mcp/server.go`, `docs/architecture/multi-agent-feedback.md`) |
| A6 | **First-class decision/notes nodes** | Lean into indexing committed markdown briefs (`HERMES-BRIEF.md`, decision docs) as queryable graph nodes, so "what did we decide about X" resolves alongside code relationships. | knowing already indexes docs; making decision/why nodes first-class lets one graph bridge code ontology + decision memory, shrinking the need for a separate notes-memory layer in agent setups. | Medium | Not started |
