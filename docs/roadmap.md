...
| 4 | **Proximity-weighted packing** | Enrichment regression (0.236->0.182 on saleor) is a packing problem. Full BFS approach tested (session 24): P@10 neutral but 2x latency, 25% memory (4-hop BFS per query on full adjacency). Reverted. Two cheaper approaches: (a) lazy BFS on top-100 post-ranking candidates only, (b) use RWR score ratio as distance proxy (`-log(score/max)/log(alpha)`, zero extra cost). | Medium | Fix enrichment regression |
