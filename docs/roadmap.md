...
| 14a | **RWR phantom dampening** | The real fix for enrichment regression. Phantoms dilute walk probability before symbols reach scoring/packing. Options: (1) exclude phantom nodes from RWR adjacency map entirely, (2) dampen phantom edge weights (0.1x), (3) filter phantom targets from edge list before walk. Need to test on enriched saleor (57K phantoms, P@10=0.182 enriched vs 0.236 unenriched). | Medium | Fix saleor enrichment regression |
