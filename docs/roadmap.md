...
| 3 | **Language-aware keyword extraction** | Task descriptions say "add a before_request hook" but the keyword extractor doesn't know "before_request" is a symbol name. The buildFTSQuery function now targets symbol_name column with compounds, but the tiered search still splits them. | +2-3pp P@10 across all repos | P1 |
