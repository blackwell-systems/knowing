...
| A3 | **Git-native auto-sync hook** | Ship a `knowing` git post-merge / post-checkout hook (or `knowing watch --roster`) that reindexes affected repos on `git pull`. | The `--watch` fs-event path assumes a long-lived process on the working tree. An agent that pulls repos needs the graph to refresh on pull with zero agent action, so "stay in sync" is turnkey instead of a per-harness cron recipe. | Low | Not started |
