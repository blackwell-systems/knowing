...
| 7 | **Redesign task memory for real learning** | Current task memory is redundant: records (keywords -> top-5 symbols) but pipeline already finds those symbols. Three directions for real learning: | Medium | Compounding |
| 7a | *Cross-task symbol association* | When task A returns Checkout and task B returns Order, and both are useful, learn the association. Next time either appears, boost the other. Pipeline can't derive this: BM25 doesn't know "checkout" and "order" are related. | Medium | New reachability |
| 7b | *Negative feedback (demote noise)* | When agent uses symbols 1,3 but ignores 2,4,5 from the top-5, record negatives. Demote ignored symbols for similar future queries. Current system only records positives. | Low | Precision |
