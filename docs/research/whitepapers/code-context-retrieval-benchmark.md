> **Note (session 25, 2026-06-02):** Numbers updated to reflect current state. P@10 = 0.293 (300 tasks, 16 repos, 8 languages, 13 self-adapting mechanisms). Key changes since initial draft: framework equivalence classes with forced injection (+57% P@10), per-cluster implicit feedback (R@10 +5.2%, MRR +12.6% compounding), vocabulary expansion from agent usage, FTS fallback decomposition, LSP edge attenuation (0.3x for lsp_resolved), change-aware scoring, adaptive proximity exponent. Corpus expanded from 277 to 300 tasks across 16 repos. Embeddings confirmed neutral. Task memory disabled (contamination found session 23).
...
Key findings: (1) graph-based retrieval (knowing) achieves P@10=0.293, 3.37x
