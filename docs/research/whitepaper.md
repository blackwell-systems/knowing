...
**The benchmark comes from two live codebases plus synthetic tests.** The primary evaluation uses the knowing codebase (~70K LOC Go, ~13.1K edges, 57 packages). A second validation uses Spring PetClinic (Java/Spring Boot, 47 source files, 5,522 nodes, 3,048 edges, 21 HTTP routes detected via Spring annotation extraction). The Java validation confirms that the hierarchical structure works across language ecosystems with different package granularity (Java packages are typically finer-grained than Go packages). Synthetic tests extend coverage to 100K edges with controlled parameters. The speedup ratios should be treated as directionally correct for graphs where packages are natural query boundaries. Validation on significantly larger codebases (100K+ LOC non-Go) remains future work.
