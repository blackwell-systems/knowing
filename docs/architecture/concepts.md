...
| **Edge** | A relationship between two nodes. 38 edge types (calls, imports, implements, extends, tests, handles_route, publishes, subscribes, documents, gated_by_flag, contains, member_of, co_tested_with, type_hint_of, accesses_field, reads_env, executes_process, consumes_endpoint, etc.). Carries a type, confidence score, and provenance. See [Edge Types](edge-types.md). | `sha256("edge\0" \|\| source_hash \|\| target_hash \|\| edge_type \|\| provenance)` |
...
At runtime, framework-specific classes with high confidence (weight >= 0.9, source "framework") bypass RWR scoring entirely and inject directly into the ranked results (**forced injection**). This solves the vocabulary gap: when the task says "custom validator" but the symbol is `EmailValidator`, BM25 can't bridge the gap but the equiv class maps the concept directly to the symbol. The injection happens post-RWR, pre-packing, ensuring relevant symbols appear in the output regardless of graph walk results.

**Learned vocabulary expansion (sessions 25-26):** When an agent uses a symbol after a `context_for_task` query, the keyword -> symbol association is recorded in `vocab_associations`. After 2+ observations, the association becomes a learned equivalence class that competes through RRF (**soft injection**, not forced). A noise filter (`isVocabWorthy`) removes ~80 common English words from recording. Confidence weighting scales RRF weight from 0.3 (count=2) to 0.8 (count>=10), rewarding reinforced associations. Per-cluster scoping (keyword_cluster on feedback table) prevents cross-task interference. Cross-task bridging validated: task A's vocab helps task B via shared keywords (Django +41.4%, full corpus 0.0% aggregate). This is the primary active learning mechanism (mechanism #13).
