...
**Benchmark:** Cross-system benchmark (14 repos, 277 tasks, 8 languages): P@10=0.283 cold start. 2.10x vs codegraph (19K stars), 3.77x vs GitNexus, 4.49x vs Gortex, 21.8x vs grep. Query latency: 2ms on k8s with adjacency cache. Focused seed selection + cluster-aware gap-fill. Parameter sweep proved all RWR/ranking parameters are irrelevant (identical P@10 across 26 configs); P@10 is reachability-determined, not ranking-determined. See [Retrieval Pipeline](retrieval-pipeline.md) for the full architecture reference.
