...
**Benchmark:** Cross-system benchmark (12 repos, 222 tasks, 7 languages): P@10=0.223 cold start, 0.249 with task memory compounding (+11.5%). 1.65x vs codegraph (19K stars), 2.97x vs GitNexus, 3.54x vs Gortex, 17.2x vs grep. Query latency: 2ms on k8s with adjacency cache. Parameter sweep proved all RWR/ranking parameters are irrelevant (identical P@10 across 26 configs); P@10 is reachability-determined, not ranking-determined. See [Retrieval Pipeline](retrieval-pipeline.md) for the full architecture reference.
