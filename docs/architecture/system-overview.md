...
**Benchmark:** Cross-system benchmark (16 repos, 300 tasks, 8 languages): P@10=0.293 cold start. 13 self-adapting mechanisms. LSP edge attenuation (0.3x for lsp_resolved). Per-cluster implicit feedback with vocabulary expansion. FTS fallback decomposition. Query latency: 2ms on k8s with adjacency cache. See [Retrieval Pipeline](retrieval-pipeline.md) for the full architecture reference.
