...
**Benchmark:** Cross-system benchmark (14 repos, 277 tasks, 8 languages): P@10=0.189 cold start. 2.17x vs codegraph (19K stars), 3.44x vs GitNexus, 3.63x vs Gortex, 12.6x vs grep. Query latency: 2ms on k8s with adjacency cache. Focused seed selection + cluster-aware gap-fill. Parameter sweep proved all RWR/ranking parameters are irrelevant (identical P@10 across 26 configs); P@10 is reachability-determined, not ranking-determined. See [Retrieval Pipeline](retrieval-pipeline.md) for the full architecture reference.
