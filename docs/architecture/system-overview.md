...
**Benchmark:** Cross-system benchmark (12 repos, 237 tasks, 7 languages): P@10=0.257 cold start, 0.262 with task memory compounding (+3.7%). 1.90x vs codegraph (19K stars), 3.43x vs GitNexus, 4.08x vs Gortex, 19.8x vs grep. Query latency: 2ms on k8s with adjacency cache. Parameter sweep proved all RWR/ranking parameters are irrelevant (identical P@10 across 26 configs); P@10 is reachability-determined, not ranking-determined. See [Retrieval Pipeline](retrieval-pipeline.md) for the full architecture reference.
