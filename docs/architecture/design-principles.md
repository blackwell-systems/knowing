...
**Framework equivalence classes with forced injection (session 23):** 263 concept-to-symbol mappings across 30 per-framework files. High-confidence matches (weight >= 0.9, source "framework") bypass RWR scoring and inject directly into ranked results (forced injection). Learned vocabulary associations (source "learned") compete through RRF (soft injection) with confidence-weighted scoring (0.3 at count=2, 0.8 at count>=10). Language-scoped via `Lang` field to prevent cross-language false positives. This is the primary mechanism for bridging the vocabulary gap (+57% P@10). Learned vocab from agent usage (sessions 25-26) bridges across tasks: task A's learning helps task B via shared keywords. Associations expire per-package via Merkle roots.
...
- P@10 = 0.321 cold start across 291 tasks, 16 repos, 8 languages (Go, Python, TypeScript, Rust, Java, C#, Ruby), 14K to 3.5M LOC. 13 self-adapting mechanisms.
- Competitive advantage (cold): vs codegraph 3.69x, vs GitNexus 5.84x, vs Gortex 6.17x, vs Aider 13.96x, vs grep 21.4x
