...
The snapshot is the root of a four-level hierarchical Merkle tree (repo root -> package roots -> edge-type roots -> edge leaves), implemented in `internal/snapshot/hierarchical.go`. The flat tree was dropped after the hash domain prefix change made backward compatibility moot; the hierarchical root is the canonical snapshot identity. The hierarchical structure enables `DiffHierarchicalTrees` to compare package roots instead of all edges: 114x faster on the knowing repo (11K edges), 517x on 100K synthetic edges. `SubgraphRoot` computes O(1) cache keys for any package set. `EdgeTypeRoot` answers "did call edges change?" in one lookup. Build cost is comparable to the flat tree.
