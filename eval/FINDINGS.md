...
| Add a method to SQLiteStore that queries nodes by communi... | 20.0% | 33.3% | 0.12 | easy |
...
| Implement garbage collection for old snapshots beyond ret... | 20.0% | 50.0% | 1.00 | easy |
| Add full-text search to the store layer for finding nodes... | 60.0% | 150.0% | 1.00 | easy |
| Register a new language extractor with the indexer | 20.0% | 40.0% | 1.00 | easy |
...
| Query edges coming into and out of a node in the store | 40.0% | 100.0% | 0.33 | easy |
| Expose community detection results through the MCP server | 0.0% | 0.0% | 0.00 | easy |
| Batch insert multiple nodes into the SQLite store | 60.0% | 120.0% | 0.20 | easy |
...
| After indexing, enrich extracted edges with type informat... | 80.0% | 133.3% | 1.00 | medium |
| Given a set of file paths, retrieve relevant symbols usin... | 0.0% | 0.0% | 0.00 | medium |
| Handle an index-repo request from the MCP protocol, runni... | 90.0% | 150.0% | 0.50 | medium |
...
| Compare two graph snapshots and report which nodes and ed... | 50.0% | 83.3% | 1.00 | medium |
| Watch for git commits and trigger re-indexing of changed ... | 20.0% | 33.3% | 0.33 | medium |
...
| Find affected tests by tracing the call graph backward fr... | 50.0% | 71.4% | 1.00 | medium |
| Wire feedback scoring into the context engine so that pre... | 0.0% | 0.0% | 0.00 | medium |
| Compute a semantic diff between two graph snapshots showi... | 40.0% | 66.7% | 0.50 | medium |
...
| When an agent reports that retrieved context was unhelpfu... | 20.0% | 25.0% | 1.00 | hard |
| Index multiple repositories, resolve cross-repo symbol re... | 0.0% | 0.0% | 0.00 | hard |
| Detect changed files from git, clean up stale nodes and e... | 10.0% | 11.1% | 0.20 | hard |
| Detect when the stored graph has drifted from the actual ... | 20.0% | 25.0% | 1.00 | hard |
...
| Implement a hybrid retrieval strategy that combines BM25 ... | 10.0% | 12.5% | 0.12 | hard |
...
| Given a task description, seed the graph walk from keywor... | 50.0% | 50.0% | 1.00 | hard |

## Per-Tier Summary

| Tier | Precision@10 | Recall@10 | MRR | Fixtures |
|------|-------------|-----------|-----|----------|
| easy | 27.0% | 69.3% | 0.48 | 20 |
| medium | 31.5% | 50.4% | 0.48 | 20 |
| hard | 18.0% | 21.3% | 0.50 | 15 |
| **Overall** | **26.2%** | **49.3%** | **0.48** | **55** |
