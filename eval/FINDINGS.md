...
| Add a method to SQLiteStore that queries nodes by communi... | 70.0% | 116.7% | 0.33 | easy |
...
| Create a new snapshot of the current graph state | 20.0% | 50.0% | 1.00 | easy |
| Fix keyword extraction logic in the context engine | 20.0% | 66.7% | 1.00 | easy |
...
| Batch insert multiple nodes into the SQLite store | 70.0% | 140.0% | 0.33 | easy |
| Compute a semantic diff between two versions of a file | 20.0% | 100.0% | 1.00 | easy |
| Pack ranked symbols into a token budget for the context w... | 30.0% | 75.0% | 1.00 | easy |
...
| Combine vector similarity search with graph-based ranking... | 10.0% | 16.7% | 1.00 | medium |
...
| Implement HITS hub/authority reranking in the context eng... | 40.0% | 50.0% | 1.00 | medium |
...
| Bootstrap the system from scratch: parse a Go codebase wi... | 10.0% | 12.5% | 1.00 | hard |
...
| Detect when the stored graph has drifted from the actual ... | 60.0% | 75.0% | 0.20 | hard |
| Before an agent renames a function, check all downstream ... | 0.0% | 0.0% | 0.00 | hard |
| Ingest OpenTelemetry spans and create runtime edges with ... | 20.0% | 25.0% | 0.50 | hard |
| Implement a hybrid retrieval strategy that combines BM25 ... | 10.0% | 12.5% | 0.17 | hard |
...
|------|-------------|-----------|-----|----------|
| easy | 37.5% | 92.7% | 0.72 | 20 |
| medium | 31.5% | 50.7% | 0.72 | 20 |
| hard | 20.0% | 23.7% | 0.40 | 15 |
| **Overall** | **30.5%** | **58.6%** | **0.63** | **55** |
