...
- **Multi-agent feedback semantics** (roadmap A5): defined and enforced how the learned-feedback / noise-demotion layer behaves when multiple agents or harnesses share one graph (now real via `knowing mcp --http`). The MCP server's ephemeral attribution state (`lastTaskKeywords`, `lastPacks`, session-recency boosts, implicit-feedback tracker, GCF dedup) is now **per-connection**, keyed by MCP session id and evicted on disconnect. This fixes a data race (concurrent map writes could panic the process) and silent cross-agent attribution corruption (one agent's edits were credited under another's task keywords). Durable feedback stays **pooled** across agents (collective learning) but now records **provenance** — the client/harness name from `initialize` instead of a hardcoded `"implicit"` — so a per-agent read policy is a future read-path change with no migration. Poison-resistance (confidence weighting + Merkle-root expiration) documented as an explicit threat model. See `docs/architecture/multi-agent-feedback.md`.

### Changed

- **npm publishing migrated to OIDC trusted publishing.** npm permanently revoked classic tokens (2025-12-09) and caps granular tokens at 90 days, so the token-based npm release was broken/expiring. The npm job in `release.yml` now publishes via OIDC (`id-token: write`, no `NODE_AUTH_TOKEN`) on Node 22 with npm upgraded to >= 11.5.1; provenance is generated automatically (all root + platform `package.json` files carry a `repository` field so provenance validates). The redundant manual `npm-publish.yml` was removed (one trusted-publisher entry per package). PyPI continues to publish via API token (PyPI has not revoked tokens); moving it to OIDC is a follow-up. **Requires one-time setup:** a Trusted Publisher on npmjs.com for each of the 5 packages, keyed on owner/repo/workflow filename.
- **GitHub release notes** are now assembled from the tag's `CHANGELOG.md` section (was a static "see CHANGELOG" pointer).
