...
- **Release publishing migrated to OIDC trusted publishing.** npm permanently revoked classic tokens (2025-12-09) and caps granular tokens at 90 days, so the token-based release workflow was broken/expiring. The npm and PyPI jobs in `release.yml` now publish via OIDC (`id-token: write`, no `NODE_AUTH_TOKEN`/`TWINE_PASSWORD`), on Node 22 with npm upgraded to >= 11.5.1; npm provenance is now automatic. The redundant manual `npm-publish.yml` was removed, consolidating to one publishing workflow (one trusted-publisher entry per package). **Requires one-time setup:** a Trusted Publisher configured on npmjs.com for each of the 5 packages (and on pypi.org for the PyPI project), keyed on owner/repo/workflow filename.
