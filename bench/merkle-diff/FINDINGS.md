...
|-----------|-----------|----------|
| Flat | 2.818959ms | baseline |
| Hierarchical | 4.759084ms | +68.8% |
...
|-----------|------------|--------|
| Flat diff (compare all 15163 edges) | 956.114µs | O(edges) |
| Hierarchical diff (compare 60 package roots) | 4.794µs | O(packages) |
| **Speedup** | **199x** | |
...
| EdgeTypeRoot ("calls") | 12.559µs | "Did any call edges change?" |
...
1. **Diff is O(packages) not O(edges).** Comparing 60 package roots instead of
   15163 edge leaves produces a 199x speedup.
