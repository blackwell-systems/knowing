...
- **Edges:** 12328 unique
- **Packages:** 115
- **Edge types:** 2 (calls:12204, throws:124)
- **Mutation target:** github.com/blackwell-systems/knowing/internal/mcp (1435 edges mutated, 11.6% of total)

## Build Cost

| Tree type | Build time | Overhead |
|-----------|-----------|----------|
| Flat | 2.0865ms | baseline |
| Hierarchical | 3.130709ms | +50.0% |
...
|-----------|------------|--------|
| Flat diff (compare all 12328 edges) | 841.715µs | O(edges) |
| Hierarchical diff (compare 115 package roots) | 6.446µs | O(packages) |
| **Speedup** | **131x** | |
...
|-----------|------------|-----------------|
| SubgraphRoot (1 package) | 72ns | Cache key for queries scoped to one package |
| EdgeTypeRoot ("calls") | 19.842µs | "Did any call edges change?" |
...
1. **Diff is O(packages) not O(edges).** Comparing 115 package roots instead of
   12328 edge leaves produces a 131x speedup.
...
graph with 20 packages gets 283x. The knowing repo (12328 edges, 115 packages) gets
131x.
