...
|-----------|-----------|----------|
| Flat | 3.539375ms | baseline |
| Hierarchical | 5.675791ms | +60.4% |
...
|-----------|------------|--------|
| Flat diff (compare all 18137 edges) | 1.157971ms | O(edges) |
| Hierarchical diff (compare 60 package roots) | 5.873µs | O(packages) |
| **Speedup** | **197x** | |
...
| EdgeTypeRoot ("calls") | 15.737µs | "Did any call edges change?" |
...
1. **Diff is O(packages) not O(edges).** Comparing 60 package roots instead of
   18137 edge leaves produces a 197x speedup.
