...
func (e *ContextEngine) ForTask(ctx stdctx.Context, opts TaskOptions) (*ContextBlock, error) {
	...
	// Reachability gap injection: BM25 results that RWR couldn't reach,
	// filtered by embedding similarity to the task description.
	// BM25 alone is too noisy (keyword matches that aren't relevant).
	// The embedding model scores each gap candidate against the task to
	// filter noise and only inject genuinely relevant unreachable symbols.
	...
	if len(bm25Results) > 0 && len(inputs) > 0 && GraphNodeCount > 10000 {
		...
		// Collect gap candidates (BM25 hits not in RWR set).
		var gapCandidates []types.Node
		for _, node := range bm25Results {
			if rwrSet[node.NodeHash] {
				continue
			}
		...
		if len(gapCandidates) > 0 {
			...
			if reranker, ok := e.vector.(VectorReRanker); ok && opts.TaskDescription != "" {
				...
				candidateHashes := make([]types.Hash, len(gapCandidates))
				for i, n := range gapCandidates {
					parts := []string{n.Kind, n.QualifiedName}
					if n.Signature != "" {
						parts = append(parts, n.Signature)
					}
					...
					candidateTexts[i] = strings.Join(parts, " ")
					candidateHashes[i] = n.NodeHash
				}

				scores, err := reranker.ReRankByHashes(ctx, opts.TaskDescription, candidateHashes, candidateTexts)
				if err == nil && len(scores) == len(gapCandidates) {
					// Only inject candidates with similarity > 0.3 (meaningful match).
					medianScore := 0.0
					if len(inputs) > 0 {
						medianScore = float64(inputs[len(inputs)/2].CallerCount)
					}
					gapCount := 0
					for i, score := range scores {
						...
						inputs = append(inputs, ScoringInput{
							Node:               gapCandidates[i],
							CallerCount:        int(medianScore),
							Confidence:         0.5,
				...
				// No embedding model: inject top BM25 gap candidates directly.
				medianScore := 0.0
				if len(inputs) > 0 {
					medianScore = float64(inputs[len(inputs)/2].CallerCount)
				}
				gapCount := 0
				for _, node := range gapCandidates {
					if gapCount >= maxGap {
						break
					}
					inputs = append(inputs, ScoringInput{
